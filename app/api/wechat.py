...
def unified_order(order, appid, mch_id, key, openid, notify_url, device_info='WEB', trade_type='JSAPI', time_out=6):
    ...
    data['body'] = '小麦芬团购接龙'
    data['detail'] = ' '.join([p.product.product.name + ':' + str(p.amount) for p in order.products]) # 商品简单描述
    data['attach'] = '青岛李沧宝龙城市广场'
    data['total_fee'] = int(order.real_price * 100) # 转换成分
...
def unified_order_js_config(appid, order, url, key):
